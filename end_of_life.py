        ...
        for table_idx, table in enumerate(tables):
            ...
            if product_col is not None and any(col is not None for col in [announcement_col, end_of_sale_col, end_of_support_col]):
                ...
                for row_idx, row in enumerate(rows[1:], 1):
                    ...
                    model_match = re.search(r'(VMX\d+|MR\d+(?:-\d+)?|MS\d+(?:-\d+(?:LP|FP|UP|X)?)?|MX\d+(?:[A-Z])?|MV\d+(?:[A-Z])?|MG\d+(?:[A-Z])?|MT\d+(?:[A-Z])?|Z\d+(?:[A-Z])?)', product_text, re.IGNORECASE)
...
def get_base_model(model):
    ...
    hyphen_match = re.match(r'((?:MR|MS|MX|MV|MG|MT|Z)\d+-\d+)', model)
    if hyphen_match:
    ...
    base_match = re.match(r'((?:MR|MS|MX|MV|MG|MT|Z)\d+)', model)
    if base_match:
