...
def get_base_model(model):
    ...
    import re
    # Look for standard Meraki model patterns
    match = re.match(r'(MR\d+|MS\d+|MX\d+|MV\d+|MG\d+|MT\d+|Z\d+)', model)
    if match:
...
def add_recommendations_to_slide(slide, recommendations, x, y, width, height):
    ...
    for i, recommendation in enumerate(recommendations[:5]):  # Limit to 5 recommendations
        ...
        # Identify if this is a product adoption recommendation
        is_product_adoption = any(product in recommendation for product in 
        ...
        # Apply blue color to non-product adoption recommendations
        if is_health_rec and not is_product_adoption:
            rec_p.text = f"• {formatted_text}"
            rec_p.font.bold = True  # Make health recommendations bold
            rec_p.font.color.rgb = ACCENT_COLOR  # Highlight important health recommendations (blue)
        else:
            # Product adoption recommendations in black (default)
            rec_p.text = f"• {formatted_text}"
...
def generate_insights(inventory_devices, dashboard_stats, firmware_stats, eol_data, products, health_score=None):
    ...
    all_recommendations = health_recommendations + general_recommendations + product_recommendations
