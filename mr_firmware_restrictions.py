...
MR_UNRESTRICTED_MODELS = [
    "MR36", "MR46", "MR45", "MR55", "MR56", "MR76", "MR86", "CW9"
]
    ...
    try:
        ...
        for doc_url in doc_urls:
            try:
                        ...
                        if product_col is not None and max_firmware_col is not None:
                            ...
                            for row in rows[1:]:
                                ...
                                if len(cells) > max(product_col, max_firmware_col):
                                    ...
                                    mr_models = re.findall(r'(MR\d+\w*)', product_text)
                ...
                # Pattern for unrestricted models
                unrestricted_pattern = re.compile(r'(MR\d+\w*).*?(?:can|will).*?(?:run|support).*?(?:current|latest|newest)', re.IGNORECASE)
                ...
                # Pattern for restricted models
                restricted_pattern = re.compile(r'(MR\d+\w*).*?(?:restricted|limited|maximum).*?(?:firmware|version).*?(\d+(?:\.\d+)?)', re.IGNORECASE)
                    ...
                    if ('mr' in header_text or 'access point' in header_text) and any(term in header_text for term in ['firmware', 'version', 'restriction']):
                        ...
                        while next_elem and next_elem.name not in ['h1', 'h2', 'h3', 'h4', 'h5']:
                            if next_elem.name in ['p', 'ul', 'ol', 'div']:
                                ...
                                # Look for MR models mentioned
                                mr_models = re.findall(r'(MR\d+\w*)', elem_text)
...
def get_model_firmware_version(model, firmware_restrictions, unrestricted_models):
    ...
    # For Cisco Wireless models, treat them as unrestricted
